/* This master script creates the schema for the AdventureWorksDW database and loads all the tables from a series of .csv files in blob storage.
*/
--> Optional - if you want a different database master key password that is fine, but the sas token below should be left as is.
CREATE MASTER KEY ENCRYPTION BY PASSWORD = 'AzureR0cks!!!';
GO
 /*
 Note:  I've created a read only sas token with an expriration in 2032, if anyone is still using this script in 10 years I'll be shocked.
 */
CREATE DATABASE SCOPED CREDENTIAL MyAzureBlobStorageCredential
 WITH IDENTITY = 'SHARED ACCESS SIGNATURE',
 SECRET = 'sp=r&st=2022-08-03T18:30:52Z&se=2032-08-04T02:30:52Z&spr=https&sv=2021-06-08&sr=c&sig=JZqiBziOAOGDeTJ13aDrgnI%2BaZQ7IaSw3gh69wMARkE%3D';
GO
...
WITH ( TYPE = BLOB_STORAGE,
          LOCATION = 'https://cgmaiadjacent.blob.core.windows.net/sourcefiles'
...
-- Load data
-- ******************************************************
BULK INSERT [DimAccount] FROM 'DimAccount.csv'
WITH (
    CHECK_CONSTRAINTS,
   -- CODEPAGE='ACP',
